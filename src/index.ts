...
let data: Solarsystem;
// **************************************************************//

//funktion för att hämta API-nyckel och sedan data från ett API
async function getBodiesData(): Promise<Planet[]> {
  ...
  let keyData: Key = await response.json();
...
function printInfo(data: Planet[], num: number): void {
  ...
  if (data[num].moons.length === 0) {
    ...
    newMoonList.forEach((moon: string) => {
      const moonListItem = document.createElement("li") as HTMLLIElement;
...
function planetClick(color: string, index: number): void {
  stars();
...
function stars(): void {
  let ctx = (canvas as HTMLCanvasElement).getContext(
    "2d"
  ) as CanvasRenderingContext2D; //skapar en 2D kontext för canvasen. används för att sen kunna rita
  ...
  ); //nollställer stjärnorna så det skapas nya varje gång funktionen körs
  ...
  for (let i = 0; i < 200; i++) {
    ...
    let x = Math.random() * (canvas as HTMLCanvasElement).width; //slumpmässigt värde på bredden
    let y = Math.random() * (canvas as HTMLCanvasElement).height; //slumpmässigt värde på höjden
